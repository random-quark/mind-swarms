class Palette {
  ...
  Palette(int _width, int _height) {
    ...
    for (Iterator i=list.iterator(); i.hasNext();) {
      ...
      for (int y=0; y<height; y++) {
        ...
        colorMode(RGB);
        color tc = colorlist.get(1).toARGB();
        palette[x][y] = lerpColor(color(255), tc, noise);
  ...
  void generateColorList() {
    ...
    pushStyle();
    colorMode(HSB,1.0);
    c = TColor.newHSV(random(1), 1.0, 1.0);
    ...
    //colorlist = new ColorRange(colorlist).getColors(2);
  ...
  color getColor(PVector location) {
    ...
    return color(hue(palette[(int) location.x][(int) location.y]), paletteSat[(int) location.x][(int) location.y], paletteBri[(int) location.x][(int) location.y]);
  ...
  void draw() {
     ...
     noStroke();
      fill(lerpColor(color(255), colorlist.get(1).toARGB(), 1));
      rect(0,0,100,100);    
  }
}